...
- Bother you with question yes-or-no only one time.
...
(defun my-git-commit-on-save ()
  "Automatically stage and commit/amend the current file on save using full commit body."
  (interactive)
  (when (and buffer-file-name (vc-backend buffer-file-name))
    (let* ((file (shell-quote-argument buffer-file-name))
           ;; Fetch the full commit body/message for this file
           (info (string-trim (shell-command-to-string (format "git log -1 --pretty=%%B -- %s" file))))
           (today (format-time-string "%F")))

      ;; Always stage the current file changes first
      (shell-command-to-string (format "git add %s" file))

      ;; If the last commit message starts with "org-history [today's date]", amend it
      (if (string-prefix-p "org-history" info)
          (progn
            (shell-command-to-string "git commit --amend --allow-empty --no-edit --date=now")
            (message "VC-Git: Amended today's commit."))
        ;; Otherwise, make a new commit with the date stamped in the message
        (progn
          (shell-command-to-string (format "git commit -m \"org-history %s\"" today))
